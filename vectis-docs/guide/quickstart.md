...
Ce guide vous permettra d'effectuer votre premier transfert Vectis en moins de 15 minutes.

## Prérequis

- Docker et Docker Compose
- Accès à un serveur Vectis (votre banque ou notre serveur de test)

## 1. Lancer le client Vectis
...
docker run -d \
  --name vectis-client \
  -p 9081:9081 \
  -e SPRING_DATASOURCE_URL=jdbc:h2:mem:vectis \
...
```bash
# Lancer un serveur Vectis de test
docker run -d \
  --name vectis-server \
  -p 5000:5000 \
  -p 8080:8080 \
